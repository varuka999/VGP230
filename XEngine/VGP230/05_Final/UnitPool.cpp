...
Unit* UnitPool::GetUnit(UnitEnum type)
{
    //if (mUnitsPool.empty())
    //{
    //    return nullptr; // Should never happen unless Load was not called
    //}

    //Unit* unit = mUnitsPool[mNextAvailableIndex];

    //while (unit->IsActive())
    //{
    //    if (mNextAvailableIndex != mUnitsPool.size() - 1)
    //    {
    //        ++mNextAvailableIndex;
    //        unit = mUnitsPool[mNextAvailableIndex];
    //        continue;
    //    }
    //    else
    //    {
